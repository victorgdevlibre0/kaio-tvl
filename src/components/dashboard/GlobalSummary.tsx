    ...
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
      {kpiCards.map((kpi) => {
        ...
        return (
          <div
            key={kpi.label}
            className={`glass-card rounded-lg p-5 transition-all duration-200 hover:border-primary/30 ${kpi.accent ? "card-glow" : ""}`}
          >
            ...
            <p className={`text-2xl lg:text-3xl font-semibold text-money ${kpi.accent ? "text-accent" : ""}`}>
              {kpi.getValue(data)}
            </p>
          </div>
        );
